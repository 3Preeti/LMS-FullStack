...
// Student management backed by localStorage (no backend dependency)
const ManageStudents = () => {
  ...
  const toggleStatus = (id) => {
    const updated = students.map((s) => (s.id === id ? { ...s, status: s.status === "Active" ? "Inactive" : "Active" } : s));
    setStudents(updated);
    localStorage.setItem("studentList", JSON.stringify(updated));
  };
      ...
      {formOpen && (
        <div style={styles.formCard}>
          ...
          <div style={styles.formRow}>
            <input type="text" placeholder="Full Name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} style={styles.input} />
            <input type="email" placeholder="Email Address" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} style={styles.input} />
          </div>
          <div style={styles.formRow}>
            <input type="text" placeholder="Roll Number" value={form.rollNumber} onChange={(e) => setForm({ ...form, rollNumber: e.target.value })} style={styles.input} />
            <input type="text" placeholder="Section" value={form.section} onChange={(e) => setForm({ ...form, section: e.target.value })} style={styles.input} />
          </div>
          <input type="text" placeholder="Batch Year (e.g., 2020)" value={form.batch} onChange={(e) => setForm({ ...form, batch: e.target.value })} style={styles.input} />
      ...
      <div style={styles.filterBar}>
        ...
        {["All", "Active", "Inactive"].map((status) => (
          <button
            ...
            style={{
              ...styles.filterBtn,
              background: filterStatus === status ? "#28a745" : "#e4e8f0",
              color: filterStatus === status ? "#fff" : "#17212b",
            }}
